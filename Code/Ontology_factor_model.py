...
folds = ['', '_fold_2', '_fold_3', '_fold_4', '_fold_5']
fold_ids = []
for fold in folds[:4]:
  with open('../qing_training/Training_queries_for_glove_dl_new' + fold + '.txt','r') as infile:
    for line in infile:
      fold_ids += [line.strip()]

train_query_ids = [str(x) for x in range(1,51)]
train_query_ids = list(filter(lambda l:l not in (fold_ids), train_query_ids))
train_query_ids = random.sample(train_query_ids, 10)
train_query_ids_dict = dict(zip(train_query_ids,range(len(train_query_ids))))
print (train_query_ids_dict)
print (train_query_ids)
with open('../qing_training/Training_queries_for_glove_dl_new_fold_5.txt','w') as outfile:
  for line in train_query_ids:
    outfile.write(line+'\n')
...
class Feedforward(nn.Module):
    def __init__(self, num_input_features, num_classes=2):
        ...
        self.fc1 = nn.Sequential(
                  nn.Linear(num_input_features-3,500),
                  nn.ReLU(),
                  )
        '''
        ...
        '''
        self.linear1 = nn.Linear(500, 3)
        self.linear2 = nn.Linear(3, num_classes)
    def forward(self, x):
        #print (x.size)
        x_query = x[:,:-3]
        x_scores = x[:,-3:]
        #print (x_query.size())
        #print (x_scores.size())
        out = self.fc1(x_query)
        out_f = self.linear1(out)
        #out =self.fc2(out)
        #print (out_f.data[0])
        #print (x_scores.data[0])
        out = out_f*x_scores
        #print (out.data[0])
...
for i, (images, labels) in enumerate(train_loader):
  ...
  print (labels.type())
  outputs,outputs_f = model(images)
  print (outputs.size())
...
for epoch in range(num_epochs):
    ...
    for i, (queries, targets) in enumerate(train_loader):
        ...
        #print (targets.type())
        # Forward pass
        outputs,outputs_f = model(queries)
        ...
        if (i+1) % 100 == 0:
            ...
            torch.save(model,'../LC_related_data_2/Pytorch_models/OF_model_temp_new_fold5.model')
...
print ('Saving...')
torch.save(model,'../LC_related_data_2/Pytorch_models/OF_model_new_fold5.model')
...
with torch.no_grad():
    outfile = open('../LC_related_data_2/Pytorch_models/ontology_factors_new_fold5.txt' , 'w')
    i = 0 
    correct = 0
    total = 0
    for queries, labels in test_loader:
        ...
        scores = outputs[:,1]
        #print (scores.shape)
        outputs_f = outputs_f.cpu().numpy()
        for idx, (qid,docid) in enumerate(X_test_order[i:i+outputs.shape[0]]):
            ranked_scores[qid] += [(docid, str(scores[idx]))]
            outfile.write(qid + ','+ docid+','+str(outputs_f[idx,0]) + ',' + str(outputs_f[idx,1]) + ',' + str(outputs_f[idx,2]) +'\n')
        i = i +outputs.shape[0]

    print('Test Accuracy: {} %'.format(100 * correct / total))
    outfile.flush()
    outfile.close()
with open('../LC_related_data_2/Pytorch_models/OF_model_predictions_new_fold5.json', 'w') as outfile:
